	...
	try:
		...
		with conn.cursor() as cur:
			query = """
                    ...
                    VALUES (%s, (SELECT id FROM users.users WHERE login = %s), %s, %s) RETURNING article_id;
					"""
	...
	try:
		...
		with conn.cursor() as cur:
			query = """
                    ...
                    WHERE article_id = %s;
					"""
			data = (article.title, article.article_body,
					article.announcement, article.id)
			cur.execute(query, data)
			conn.commit()
...
def select_articles_by_search(
	...
	try:
		...
		with conn.cursor(cursor_factory=RealDictCursor) as cur:
			# подготовка CTE для полнотекстового и trigram поиска
			sql = """
                  ...
                  SELECT article_id, greatest(
                      similarity(title, q.rawq), similarity(announcement, q.rawq), similarity(article_body, q.rawq)
                      ) AS rank_trgm
                  FROM articles.articles, q
				  ...
				  """
			# динамическое формирование списка столбцов
			select_cols = [
				'a.article_id',
				'a.title AS title',
				'us.login AS login',
			...
			if announcement:
				select_cols.append('a.announcement AS announcement')
			select_cols.append(
			...
			sql += (
				...
				"JOIN users.users us ON a.user_id = us.id "
			...
			sql += (
				" (coalesce(fts.rank_fts, 0) * 1.0 + "
				"coalesce(trgm.rank_trgm, 0) * 0.5) > 0"
			)
			...
			rows = cur.fetchall()
			logger.info(
				"Найдено %d статей по запросу %r (announcement flag: %s)",
				len(rows), query_str, announcement
			)
